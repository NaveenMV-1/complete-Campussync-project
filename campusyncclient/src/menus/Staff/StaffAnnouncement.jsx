...
export default function StaffAnnouncement() {
  ...
  const fetchHistory = async () => {
    try {
      ...
      // Filter for Staff announcements written by this user
      setHistory(res.data.filter(ann => ann.role === "Staff" && ann.author === "Dr. Ramesh Kumar"));
    } catch (err) {
  ...
  return (
    <Container fluid className="px-4">
      ...
      <header className="welcome d-flex justify-content-between align-items-center py-3 border-bottom">
        <h4 className="mb-0">Welcome back Dr. Ramesh Kumar</h4>
